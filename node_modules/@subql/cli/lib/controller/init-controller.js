...
const child_process_1 = (0, tslib_1.__importStar)(require("child_process"));
...
async function cloneProjectTemplate(localPath, projectName, template) {
    const projectPath = path.join(localPath, projectName);
    try {
        await (0, simple_git_1.default)().clone(template.remote, projectPath, ['-b', template.branch, '--single-branch']);
    }
    catch (e) {
        let err = 'Failed to clone starter template from git';
        try {
...
async function readDefaults(projectPath) {
    ...
    const currentProject = js_yaml_1.default.load(manifest);
    return [
